...
const GsapTimeline = () => {
  ...
  const timeline = gsap.timeline({
    //One can already pass some properties:
    repeat: -1, repeatDelay: 1, yoyo: true
  })

  useGSAP(()=>{
    //timeline set, we can add multiple animations to the same target, one after another:
  ...
  return (
    <main>
      ...
      <div className="mt-20 space-y-10">
        <button onClick={() => {
          //here we can set a condition to play/pause the animation with the button:
          if (timeline.paused()) {
            timeline.play();
          } else {
            timeline.pause();
